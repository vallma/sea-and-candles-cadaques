    ...
    <form action={formAction} className="space-y-10">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div className="md:col-span-2">
          ...
          <input name="name" defaultValue={initialData?.name} required placeholder="Cargol de Mar" className={field} />
        </div>
        <div className="md:col-span-2">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">Les opcions poden afegir un +/- al preu base</p>
        </div>
        <div>
          <label className={label}>Estoc general</label>
          <input name="stock" type="number" min="0" defaultValue={initialData?.stock ?? 0} className={field} />
          <p className="text-xs text-slate-400 mt-1">Si tens opcions, l'estoc es gestiona per opció</p>
        </div>
        <div>
      ...
      {/* ── OPCIONS / VARIACIONS ── */}
      <section>
        <h2 className="text-base font-medium text-slate-800 mb-1">Opcions del producte</h2>
        <p className="text-sm text-slate-400 mb-5">
          El client podrà escollir de cada grup. Ex: <span className="text-slate-600">Olor → Vainilla</span> · <span className="text-slate-600">Mida → XL</span>
        </p>
        <OptionGroupEditor value={optionGroups} onChange={setOptionGroups} />
      </section>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <div>
          <label className={label}>Temps cremada (h)</label>
          <input name="burningTime" type="number" min="1" defaultValue={initialData?.burningTime ?? ""} placeholder="35" className={field} />
        </div>
        <div>
          <label className={label}>Pes (g)</label>
          <input name="weight" type="number" min="1" defaultValue={initialData?.weight ?? ""} placeholder="180" className={field} />
        </div>
        <div>
          <label className={label}>Dimensions</label>
          <input name="dimensions" defaultValue={initialData?.dimensions ?? ""} placeholder="8 × 8 cm" className={field} />
        </div>
      </section>

      {/* ── MATERIALS ── */}
      <section>
        <h2 className="text-base font-medium text-slate-800 mb-2">Materials</h2>
      ...
      <section>
        ...
        <p className="text-sm text-slate-400 mb-3">Per filtrar i cercar</p>
      ...
      <div className="flex items-center justify-between pt-6 border-t border-slate-100">
        ...
        <button type="submit" disabled={pending}
          ...
          {pending ? "Guardant..." : initialData ? "Actualitzar" : "Crear producte"}
        </button>
      </div>
    </form>
  );
